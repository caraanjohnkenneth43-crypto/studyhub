...
export default function DmRoom() {
  ...
  if (roomLoaded && !room) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center" style={{ background: "var(--c-bg)", color: "var(--c-subtle)" }}>
        ...
        <Link href="/dm" className="text-sm underline mt-2" style={{ color: "var(--c-link)" }}>Back to messages</Link>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center" style={{ background: "var(--c-bg)", color: "var(--c-subtle)" }}>
        <div className="rounded-xl border shadow-xl p-6 w-full max-w-sm mx-4 text-center" style={{ background: "var(--c-card)", borderColor: "var(--c-border)" }}>
          ...
          <p className="text-sm mb-4" style={{ color: "var(--c-muted)" }}>You&apos;ve been removed from this conversation by the other participant.</p>
          <Link href="/dm" className="text-sm underline" style={{ color: "var(--c-link)" }}>Back to messages</Link>
        </div>
      </div>
    )
  }
      ...
      <div className="min-h-screen flex flex-col items-center justify-center" style={{ background: "var(--c-bg)", color: "var(--c-subtle)" }}>
        <div className="rounded-xl border shadow-xl p-6 w-full max-w-sm mx-4" style={{ background: "var(--c-card)", borderColor: "var(--c-border)" }}>
          <div className="flex items-center justify-between mb-1">
            <h2 className="text-lg font-semibold" style={{ color: "var(--c-fg)" }}>🔒 Private Conversation</h2>
            <button onClick={() => router.back()} className="text-sm px-2 py-1 rounded hover:bg-black/5" style={{ color: "var(--c-muted)" }}>&larr; Back</button>
          </div>
          <p className="text-sm mb-4" style={{ color: "var(--c-muted)" }}>Enter the conversation password to join.</p>
          <input
            value={password}
          ...
          <button onClick={checkPassword} disabled={!password.trim()} className="w-full py-2 text-white rounded-lg text-sm font-medium disabled:opacity-50" style={{ background: "var(--c-accent)" }}>Join Conversation</button>
        </div>
      </div>
    )
  }
  ...
  const otherUser = members?.find(m => m.uid === otherParticipant)

  return (
    <div className="min-h-screen flex flex-col" style={{ background: "var(--c-bg)" }}>
      <Navbar
        room={room}
        onBack={() => router.push("/dm")}
        ...
        <div className="relative z-10 max-w-6xl mx-auto w-full px-4">
          <div className="rounded-xl border shadow-xl p-4 w-full max-w-sm" style={{ background: "var(--c-card)", borderColor: "var(--c-border)" }}>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-semibold" style={{ color: "var(--c-fg)" }}>Conversation Participants ({members.length})</h3>
            ...
            {members.length === 0 ? (
              ...
              <div className="space-y-1.5 max-h-80 overflow-y-auto">
                {members.map(member => {
                  const statusClass = member.status === "active" ? "bg-green-500" : member.status === "idle" ? "bg-yellow-500" : "bg-gray-500"
                  const statusDotClass = member.status === "active" ? "w-2 h-2 rounded-full bg-green-500" : member.status === "idle" ? "w-2 h-2 rounded-full bg-yellow-500" : "w-2 h-2 rounded-full bg-gray-500"
                  const statusBorderColor = "var(--c-card)"
                  const statusBorderStyle = { position: "absolute", bottom: 0, right: 0, width: "0.625rem", height: "0.625rem", borderRadius: "9999px", borderWidth: "0.125rem", borderStyle: "solid", borderColor: statusBorderColor, ...(statusClass === "bg-green-500" ? { backgroundColor: "#22c55e" } : statusClass === "bg-yellow-500" ? { backgroundColor: "#eab308" } : { backgroundColor: "#9ca3af" }) }
                  return (
